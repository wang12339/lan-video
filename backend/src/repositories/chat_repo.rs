...
impl ChatRepository {
    ...
    pub async fn insert(
        ...
        sqlx::query_as::<_, ChatMessageRow>(
            "INSERT INTO chat_messages (tenant_id, user_id, username, content, msg_type, image_url, video_url) \
    ...
    pub async fn find_by_id(
        ...
        sqlx::query_as::<_, ChatMessageRow>(
            "SELECT id, user_id, username, content, msg_type, image_url, video_url, created_at \
             FROM chat_messages WHERE id = $1 AND tenant_id = $2",
        )
        .bind(id)
        .bind(tenant_id)
    ...
    /// 本租户消息总数（管理后台统计用）。
    pub async fn count(&self, tenant_id: i64) -> Result<i64, sqlx::Error> {
        let (n,) =
            sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM chat_messages WHERE tenant_id = $1")
                .bind(tenant_id)
                .fetch_one(&self.pool)
                .await?;
        Ok(n)
    ...
    /// 清空本租户全部消息。返回删除条数。
    pub async fn delete_all(&self, tenant_id: i64) -> Result<u64, sqlx::Error> {
        let res = sqlx::query("DELETE FROM chat_messages WHERE tenant_id = $1")
            .bind(tenant_id)
            .execute(&self.pool)
            .await?;
    ...
    /// 管理员删除单条消息。返回是否删除成功。
    pub async fn delete(&self, tenant_id: i64, id: i64) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM chat_messages WHERE id = $1 AND tenant_id = $2")
            .bind(id)
            .bind(tenant_id)
            .execute(&self.pool)
            .await?;

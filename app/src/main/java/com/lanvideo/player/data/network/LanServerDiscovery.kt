...
object LanServerDiscovery {
    ...
    suspend fun discover(
        ...
        if (isLanVideoServiceReachable(current, probeClient)) {
            MyApplication.instance.setConnectionState(com.lanvideo.player.ConnectionState.CONNECTED)
            return@withContext null
        }
        val ipv4 = getIpv4OnNetwork(cm, network) ?: return@withContext null
        MyApplication.instance.setConnectionState(com.lanvideo.player.ConnectionState.SCANNING)
        ...
        val found = scanCandidates(quick, fastProbeClient)
            ...
            ?: run {
                MyApplication.instance.setConnectionState(com.lanvideo.player.ConnectionState.DISCONNECTED)
                return@withContext null
            }
        if (found != current) {
